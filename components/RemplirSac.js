        ...
        for (j = 1; j < nbItems; j++) {
          if (i < this.state.Items[j - 1].weight) {
            ...
            if (
              matriceGains[j - 1][i] >
              matriceGains[j - 1][
                i - parseInt(this.state.Items[j - 1].weight, 10)
              ] +
                parseInt(this.state.Items[j - 1].value, 10)
            ) {
              matriceGains[j][i] = parseInt(matriceGains[j - 1][i], 10);
            } else {
              matriceGains[j][i] =
    ...
    return (
      <View style={styles.container}>
        ...
        <View style={styles.buttonContainer}>
          <Button
            title="Ver lista dos Items"
            color="#f2545B"
            style={styles.button}
            onPress={() =>
              navigate('ListPris', {
                sac: {
                  items: this.state.ItemsPris,
                  poidsObtenu: this.state.poidsFinal,
                  poidsMax: this.state.poidsMax,
                  valeurObtenue: this.state.Gain,
                },
              })
            }
          />
